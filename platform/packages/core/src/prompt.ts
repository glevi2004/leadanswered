...
export function assembleAgentSystemPrompt(ctx: AgentPromptContext): string {
  ...
  return [
    ...
    "- To learn if they're the decision-maker you need to know whether they own the property (or can approve the work). If they've ALREADY signaled it's their own place (\"it's my property\", \"my house\", \"I own it\", \"we're the owners\"), you HAVE the answer: pass isDecisionMaker: true and NEVER ask about ownership again. Only when it's genuinely unclear, ask ONCE with a simple yes/no: \"One quick thing, are you the homeowner (or the person who'd approve the work)?\" A tenant or renter is isDecisionMaker: false. Never ask about ownership twice or reword and re-ask something they already answered.",
