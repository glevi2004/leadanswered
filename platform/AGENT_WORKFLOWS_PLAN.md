...
address, their name, the homeowner yes/no, the day/time), code records it and sends the next locked
...
3. **Off-script** — a question ("how much?", "can you come today?"), a comment, or info out of order →
   the agent **handles it in its own words** (answers, or `escalate_to_contractor` for price / anything
...
**1 — Homeowner** — after they send the address. Records `qualify_lead{isDecisionMaker}`:
```
Got it, thanks {name}! And are you the homeowner (or the person who'd approve the work)?
```

**2 — Offer windows** — after homeowner = yes AND `qualify()` passes. `check_availability` (general):
```
...
### Branch A — not the homeowner (they answer #1 with "no / I rent")

**A1 — ask for the owner's contact:**
```
No problem! Could you share the homeowner's name and best phone number? I'll reach out to them directly to get it set up.
```
...
**A3 — Sarah reaches out to the homeowner** — a NEW lead for the owner. Two blocks, blank line:
```
...
booking. If the owner isn't the right person either → escalate to the contractor.
...
`escalate_to_contractor`. Its prompt drops all the qualification driving — intake did that.
...
from the homeowner step on it's identical to Workflow 1.
...
NOT a new job (existing customer, a question, wrong number) → `escalate_to_contractor`, END intake:
```
...
After the name+address reply: **Homeowner** (msg 1) → `qualify()` → **Windows** (2) → **Times** (3) →
**Booking** (4), plus the same **Branch A** (not the homeowner) and **Branch B** (out of area, confirm
...
Whenever Sarah escalates (`escalate_to_contractor`), the handoff runs and is **never a dead end**:

1. **Escalate** — the contractor is texted the lead's context/question ("…reply to answer them"), and
...
2. **Contractor answers** — replies to that text (handled in Workflow 3).
...
Chain: **escalate → contractor answers → agent relays naturally → agent continues.** (The relay is a
rigid template only in today's code because there was no agent phase; now there is.)

---

## WORKFLOW 3 — Contractor  (always agent)

**Event:** a contractor texts the assistant. **No intake — agent from message one.** The "employee."

**Tools:**
- `find_leads({ name })` → the contractor's matching leads `[{ leadId, name, project, area, status }]`.
...
**Contractor-facing messages** (functional, so fairly fixed):
...
**Escalation-reply relay lives here:** when the contractor replies to an escalation text, this agent
...
```
inbound →
  sender is a contractor?         → Contractor agent
  else (a lead):
...
- **`intake/steps.website.ts`** / **`intake/steps.missedCall.ts`** — the ordered steps; each is
  ...
  Missed-call reuses the website steps from `homeowner` onward.
...
- **`agent/contractorTools.ts`** — `find_leads`, `send_message_to_lead`.
- **`agent/contractorAgent.ts`** — the owner-facing tool loop; also composes the **escalation-reply
  relay** to the lead.

### Changed files
- **`conversationService.ts`** — `handleInbound` becomes the **ROUTER**: contractor sender → contractor
  ...
  **Delete the templated relay** in `maybeRelayContractorReply` — relay moves into the contractor agent.
...
- **`agent/ownerHandoff.ts`** — `handOffOwnerIfReady` **changes behavior**: instead of only pinging the
  contractor, it **creates a new lead for the homeowner** + sends the outreach (Branch A3), which then
  runs a short intake (project+address already known). Contractor still gets a heads-up.
...
- **`packages/core/src/prompt.ts`** — split the one mega-prompt into: **(a)** extractor prompt (read the
  reply), **(b)** lead-agent prompt (non-linear), **(c)** contractor-agent prompt. `openingTrigger`
...
### Deleted
- **`contractorCommands.ts`** + **`contractorCommands.test.ts`** — replaced by the contractor agent
...
### DELETE (1)
- `contractorCommands.test.ts` — state machine gone (its coverage becomes the contractor-agent test).
...
- `agent/tools.test.ts` (7) — every tool gate survives (qualify / availability / book / reschedule /
  cancel). **ADD**: `qualify_lead` writes `name`→`contactName`. **CHANGE**: the not-homeowner test —
  `handOffOwnerIfReady` now **creates a homeowner lead + texts them** (not just a contractor ping).
...
- **`conversationService.test.ts`** (10) → split, assertions redistributed so none are lost:
  - **`router.test.ts`** ← idempotent duplicate skipped; AI-error fails safe (no SMS); cold-inbound
    (default skip / `allowColdInbound` starts a lead); contractor-sender routing.
  ...
  - **contractor-agent test** ← the escalation loop + relay (now agent-composed, not the template).
- **`leadService.test.ts`** (2) → creates the lead + **starts intake** (fires step 0), `state=intake`;
  the (contractor,phone) dedup + re-engage assertions stay.
- **`prompt.test.ts`** (6 / 23 assertions) → 3 suites: **lead-agent prompt** gets the tool + guardrail
  ...
  locked copy now, not a prompt); + new **extractor** + **contractor** prompt tests.
...
- **`intake/steps.website.test.ts`** — happy path fires the exact **LOCKED copy** in order; Branch A
  (not-homeowner → homeowner lead created + outreach); Branch B (out-of-area confirm → decline; corrected
  address → re-qualify → continue).
- **`intake/steps.missedCall.test.ts`** — opening → name+address **vs** escalate branch → merges to the
  website steps from `homeowner` on.
- **`agent/contractorAgent.test.ts`** — find_leads + disambiguation; **hard-gate send** (no delivery
...
- **`judge.ts` + `fixtures.ts`** (Sonnet judge + 4 timezone contractors) → **reuse as-is.**
- **`harness.ts`** → change ONLY the init — replace `createLeadAndGreet` with a channel-specific start
  into the new router/intake engine. Keep the whole surface (`say`, `sayRaw`, `contractorReply`,
  `status`, `appointments`, `escalations`, `offeredSlots`, `ownerSms`, `homeownerSms`).
...
- New per-workflow e2e (covering all 18 old scenarios' intent): `website`, `missedcall`, `handoff`,
  `outofarea`, `contractor`, `agentphase`. **10 guardrails to preserve** (from the audit): timezone-
...
- `qualify_lead` name→`contactName`; `handOffOwnerIfReady` creates+texts the homeowner; escalation relay
...
- **`SCOPE.md`**
  - **§Architecture v2** (top) — "Sarah is a tool-using agent" → **three workflows** (two scripted-intake
    → agent, one contractor agent). Fix the tools list (`check_availability`, + contractor tools).
  ...
  - **§5.1 Decision-maker → homeowner hand-off** — now Sarah **texts the homeowner directly** (new lead),
    not just pings the contractor. Update.
...
- **`TESTING.md`**
  ...
  - **ADD**: intake tests (website steps, missed-call steps, off-script adaptation, **name shows on the
    dashboard**, owner-handoff outreach to the homeowner).
...
2. **Missed-call workflow** (opening → name+address / escalate branch → reuse from homeowner).
3. **Contractor agent** (find_leads + send_message_to_lead + escalation relay) → delete
   `contractorCommands.ts`.
