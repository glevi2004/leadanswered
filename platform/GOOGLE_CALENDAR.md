...
> merge, outbound push, inbound push-webhook + incremental sync + reconcile, `applyOrganizationChange`,
...
A organization connects Google Calendar; from then on:
...
3. **Changes the organization makes flow back to us** — if they move or cancel an estimate in Google, our
   DB updates and (with the organization's OK) the lead is told.

Two ways a organization arrives, both one code path:
...
**A organization moving or cancelling an appointment is the same logical event whether they do it on our
dashboard, in their Google Calendar, or by texting Sarah. All three are triggers into ONE shared
handler with ONE behavior.**

```
applyOrganizationChange(appointmentId, { type: "cancel" | "reschedule", newStartIso? }, { source, pushToGoogle })
  ...
  4. Ask the OWNER to confirm before the lead is texted — the HARD GATE we already built
     (agent/organizationAgent.ts). The lead is messaged ONLY on an explicit yes.
...
- **Google:** detected via sync (§9) → Sarah SMS-es the organization: "You moved {lead}'s estimate to
  Thu 10am — want me to let them know? (yes/no)" → the existing organization-agent yes/no path sends it.
- **SMS (owner agent):** a natural third trigger later — "cancel Levi's Tuesday visit" → `find_leads`
  → same handler.

Why this matters: the dashboard has **no** cancel/reschedule action today (it's read-only — verified in
`apps/web`), and the old plan didn't cover organization-initiated changes at all. Building two-way as a
...
> Reuse: step 3+4 are exactly the owner agent's staged-draft + hard-gate confirm
...
SOURCE, (2) a SYNC TARGET, and (3) an INBOUND TRIGGER for organization-made changes — but Google is never
...
- A organization's **manual edit to one of OUR events in Google** is accepted and **mirrored into the DB**
...
- The shared **appointment-change handler** (§2) → `apps/api` (`appointmentChange.ts`), called by the
  dashboard route, the inbound reconcile, and later the owner agent.
...
- **Invite-only preserved:** access stays gated. A Google sign-in is allowed only for a organization whose
...
- **Account linking by email:** a organization who signed up with email/password and later clicks "Continue
  with Google" on the same email links to the **same** organization account (Supabase identity linking) — no
  duplicate.
...
- **Auth URL:** `access_type=offline` + `prompt=consent` + a signed **`state`** (organizationId + nonce, CSRF).
...
**`CalendarConnection` exists** (`organizationId`, `provider`, `externalCalendarId`, `accessToken`,
`refreshToken`, `tokenExpiresAt`, `status`, `@@unique([organizationId, provider])`). **Add for two-way**
(additive migration):
...
`open windows = standing weekly windows (organization-set, local tz) − (our active appointments + Google
busy blocks)`. The organization still DEFINES when they take estimates (the drag-paint calendar); Google
only prevents offering a slot they're already busy on. We do NOT derive business hours from Google.

---

## 9. Inbound: reading the organization's Google changes back (the two-way half)
...
- **Webhook = speed.** On connect, call `events.watch` on the primary calendar → Google POSTs to
  ...
  **incremental sync**: `events.list(syncToken)` → only changed events + a fresh `nextSyncToken` (first
  sync seeds the token; `410 Gone` → reseed). This is the fast path — the organization moves an event and
...
- **Maps to one of OUR appointments** (`externalEventId` match):
  - time changed → `applyOrganizationChange(reschedule, newStart, { source: "google", pushToGoogle: false })`.
  - deleted/cancelled → `applyOrganizationChange(cancel, { source: "google", pushToGoogle: false })`.
  - → mirrors to DB + asks the organization before texting the lead (§2).
- **A foreign event** (organization's personal/other) → not an appointment; it's just a busy block that
...
origin), Google's watch fires our own webhook. We must NOT reprocess our own write as a organization edit
...
**Conflict/authority:** a organization Google edit is authoritative for THAT appointment — mirror it. If it
lands on a slot our DB considers taken (rare race), accept the organization's calendar as truth for their
...
- **NEW — appointment actions (needed regardless of Google):** the appointments view gets **Cancel** and
  **Reschedule** actions → a organization-auth api route → `applyOrganizationChange(..., { source:
...
- Calendar scopes are "sensitive/restricted" → publishing to all organizations needs Google **OAuth app
...
- **Signed `state`** (HMAC) on OAuth → CSRF + binds callback to the initiating organization + short TTL.
...
- Session check on `/google/connect` so a organization connects only their own calendar.
...
- **Organization edits in Google + lead reschedules via Sarah at the same time** → last write to the DB wins
...
2. **Outbound push** — booked/rescheduled/cancelled sync to Google (`Appointment.external*` + `syncState`),
   idempotent + retried.
3. **The appointment-change action + inbound two-way** — build `applyOrganizationChange` + the dashboard
...
5. **Later:** organization-agent SMS trigger ("cancel Levi's visit"); multi-calendar; Outlook via the same port.
...
- **New `apps/api/src/appointmentChange.ts`** — the shared `applyOrganizationChange` handler (§2): DB
  change → conditional Google push → staged lead-notice draft → route the confirm. Reuses the
  organization-agent hard-gate.
...
- **New `apps/api/src/routes/appointments.ts`** — organization-auth Cancel/Reschedule → `applyOrganizationChange`.
...
- **Unit (mocked Google client):** free/busy merge subtracts correctly in `computeOpenWindows`;
  ...
  is skipped).
- **`applyOrganizationChange`:** dashboard-source cancels → DB cancelled + Google delete enqueued + draft
  staged, lead texted only after confirm; google-source cancels → DB cancelled, NO Google push, organization
...
- **E2E (`apps/api/src/e2e`):** connected organization with a busy block → Sarah won't offer it; booking
...
- **Manual:** connect a test-user account; book → appears in Google; move/cancel in Google → Sarah asks
  the organization → lead texted on yes; cancel on the dashboard → same; disconnect revokes + stops the watch.
