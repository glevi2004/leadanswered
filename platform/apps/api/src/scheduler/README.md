...
overlapping active appointments per organization, plus partial unique indexes) — not JS checks —
to make double-booking impossible even under concurrency. Availability = the organization's standing
...
We will let organizations connect Google Calendar (and others), but a provider is **never** the
...
- **`getBusyTimes`** — also merge the provider's free/busy (so we avoid the organization's personal
  events), via a `CalendarSync.getBusyTimes(organizationId, window)`.
...
The plumbing is already in place: the `CalendarConnection` model (per organization) and the unused
