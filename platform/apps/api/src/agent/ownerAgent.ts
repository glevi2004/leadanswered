...
import { sanitizeReply, type OrganizationConfig } from "@leadanswered/core";
import type { AgentDeps } from "./runner.js";
import { buildOwnerTools, type OrganizationDraft } from "./ownerTools.js";
import type { InboundResult } from "../conversationService.js";
import type { LeadContext } from "../store/types.js";

/**
 * The OWNER agent (Workflow 3) — always agent, no script. The owner texts the assistant like an
...
export function __resetOwnerAgent(): void { pending.clear(); }

/** Stage a "let the lead know" draft on the owner's hard-gate queue — the owner's next "yes"
 *  (handled by handleOwnerTurn) delivers it. Reused by the appointment-change flow so an
 *  owner-made cancel/move never texts the customer without the owner's explicit yes. */
export function stageLeadNotice(organizationId: string, d: { leadId: string; leadName: string; message: string }): void {
  setPending(organizationId, d);
}
...
function systemPrompt(c: OrganizationConfig): string {
  return [
    `You are ${c.sarahName}, the text assistant for ${c.companyName}. You are now talking to the OWNER (your boss), NOT a customer.`,
    ...
    `Once you know the exact lead and the message, call prepare_message_to_lead with the note in your own warm, natural words (do not add facts they didn't give). Then READ THE MESSAGE BACK to the owner and ask "Send it?" — do NOT claim it's sent. Code sends it only after they confirm with a yes.`,
    `If you can't find the lead, say so and ask them to double-check the name. Keep replies short and plain. Never use em-dashes.`,
  ].join("\n");
}

/** An owner's text when there's NO open escalation → treat as a command (pass a note / look up). */
export async function handleOwnerTurn(
  deps: AgentDeps,
  organization: OrganizationConfig,
  input: { fromNumber: string; body: string },
): Promise<InboundResult> {
  const text = input.body.trim();
  const reply = (m: string) => deps.sms.send(input.fromNumber, m).catch((e) => console.error("[owner] reply failed:", e));

  // Awaiting a yes/no on a staged draft — this confirmation is CODE-gated, not left to the model.
  const p = getPending(organization.id);
  if (p) {
    if (isYes(text)) {
      pending.delete(organization.id);
      ...
      try { await deps.sms.send(ctx.lead.contactPhone, p.message); } catch (e) { console.error("[owner] send to lead failed:", e); }
      await reply(`Done! Sent to ${firstName(ctx.lead.contactName)} ✅`);
      return { status: "ok" };
    }
    if (isNo(text)) { pending.delete(organization.id); await reply("No problem, I won't send it."); return { status: "ok" }; }
    pending.delete(organization.id); // neither yes nor no → treat as a fresh command below
  }

  // Fresh command → run the agent to parse / disambiguate / draft (it only STAGES; it can't send).
  const draft: OrganizationDraft = {};
  const tools = buildOwnerTools(deps.store, organization.id, draft);
  let text_out: string;
  try {
    const result = await generateText({
      model: deps.model,
      system: systemPrompt(organization),
      messages: [{ role: "user", content: text }],
      tools,
      stopWhen: stepCountIs(4),
      experimental_telemetry: { isEnabled: true, functionId: "owner-agent" },
    });
    text_out = result.text.trim();
  } catch (e) {
    console.error("[owner] agent error:", e);
    return { status: "error" };
  }

  if (draft.pending) setPending(organization.id, draft.pending);
...
export async function composeEscalationRelay(
  ...
  try {
    const result = await generateText({
      model: deps.model,
      system:
        `You are ${organization.sarahName} from ${organization.companyName}, texting the customer ${leadCtx.lead.contactName}. ` +
    ...
    console.error("[owner] relay compose failed:", e);
  }
