...
function systemPrompt(): string {
  return [
    ...
    "3b) If the app needs a DATABASE: call provision_backend(siteId) to wire the org's Supabase into the site's env vars (write your code assuming NEXT_PUBLIC_SUPABASE_URL / NEXT_PUBLIC_SUPABASE_ANON_KEY exist), and stage every schema change with run_migration — it records the SQL for the owner's approval and NEVER runs directly. Never claim a migration ran; say it awaits approval.",
...
export async function runEngineering(
  ...
  // Agent status (docs/system.md §2 row hygiene): the Engineer shows `working` for the
...
export async function confirmPublish(
  ...
  // 5) Resolve the Approval — and close the loop (docs/system.md §2): the task goes
