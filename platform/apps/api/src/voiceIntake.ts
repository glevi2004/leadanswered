...
export interface TwilioVoiceInbound {
  /** The original caller (customer) on the forwarded leg. */
  From?: string;
  /** The Twilio number the call was forwarded to — identifies the organization. */
  To?: string;
  /** Unique call id — our idempotency key (one text per call). */
  CallSid?: string;
  /** The organization's real number that forwarded (carrier-dependent; not always present). */
...
/**
 * Pick the customer's number off a forwarded call. Normally Twilio's `From` is the
 * original caller, so we prefer it — but guard against carriers that put the forwarding
 * (organization) number in `From`, in which case it would equal `To`/`ForwardedFrom` and we
 * bail rather than text the organization. We log the raw params so call #1 confirms the shape.
 */
export function resolveCallerNumber(p: TwilioVoiceInbound): string | null {
  ...
  if (!from) return null;
  if (from === to || from === forwardedFrom) return null; // From is the organization, not the caller
  return from;
}

/**
 * Missed-call text-back intake (SCOPE §9.7). A customer called the organization's real
 * phone, it went unanswered, and the carrier forwarded the call to the organization's Twilio
...
export async function handleMissedCall(
  ...
  const organization = await deps.store.getOrganizationByTwilioNumber(to);
  if (!organization) {
    console.warn(`[voice] no organization for number "${to}" — skipped`);
  ...
  const existing = await deps.store.findLeadContextByOrganizationPhone(organization.id, caller);
  ...
  console.log(`[voice] new missed-call lead ${leadId} for ${organization.companyName} from ${caller}`);
