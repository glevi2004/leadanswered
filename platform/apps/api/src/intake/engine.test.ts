...
const NULLS = {
  ...
  isHomeowner: null, ownerName: null, ownerPhone: null, chosenDayOfWeek: null, chosenDayLabel: null,
...
describe("intake engine — Workflow 1 (website) + Workflow 2 (missed call)", () => {
  it("website happy path: opening → address → homeowner → windows → times → book (+ captures the name)", async () => {
    const store = seed();
    ...
    expect(r1.reply).toMatch(/are you the homeowner/i);
  ...
  it("Branch A — not the homeowner: asks for the owner's contact, then reaches out to them as a new lead", async () => {
    const store = seed();
    const sms = new CapturingSms();
    const model = scriptedModel([
      EX({ fullAddress: ADDR, town: "Boston", zip: "02134", projectType: "roof leak", intent: "new_project", isHomeowner: false }),
    ...
    expect(r1.reply).toMatch(/homeowner's name and best phone/i);
    const r2 = await send("the owner is Jane, 617-555-9090");
    expect(r2.reply).toMatch(/reach out to Jane directly/i);

    // a NEW lead was created for the homeowner, and Sarah texted them the A3 outreach
    const owner = await store.findLeadContextByContractorPhone(TEST_CONTRACTOR_ID, "+16175559090");
  ...
  it("Branch B — out of area: confirms the address first, then declines (never books)", async () => {
    ...
    const sms = new CapturingSms();
    const model = scriptedModel([
      EX({ fullAddress: "5 Ocean St, Hyannis MA 02601", zip: "02601", projectType: "roof leak", intent: "new_project", isHomeowner: true }),
      EX({ addressConfirmed: true }),
    ]);
    const deps = { store, model, sms, email: new CapturingEmail(), now: () => NOW };
    await createLeadAndGreet(deps, { contractorId: TEST_CONTRACTOR_ID, contactName: "Pat", contactPhone: FROM });
    const send = (body: string) => handleInbound(deps, { toNumber: TO, fromNumber: FROM, body });
  ...
  it("missed call — new job → asks name + address; not a job → escalates", async () => {
    ...
    const deps = { store, model, sms, email: new CapturingEmail(), now: () => NOW };
    const lead = await createLeadAndGreet(deps, { contractorId: TEST_CONTRACTOR_ID, contactName: "Caller", contactPhone: FROM, source: "missed_call" });
    ...
    await createLeadAndGreet(deps2, { contractorId: TEST_CONTRACTOR_ID, contactName: "Caller", contactPhone: FROM2, source: "missed_call" });
