...
import { NOTIFICATION_EVENT_TYPES, type ContractorConfig } from "@leadanswered/core";
...
/** The contractor's own phone — used to assert notification/hand-off/escalation pings. */
...
export interface Conversation {
  ...
  contractor: ContractorConfig;
  homeownerPhone: string;
  /** The homeowner texts `body`; returns Sarah's reply ("" if skipped/error). */
  ...
  /** A contractor recipient texts back (e.g. answering an escalation). */
  contractorReply(body: string, fromRecipientPhone: string): Promise<{ status: string; reply?: string }>;
  ...
  /** SMS sent to contractor recipients (notifications, hand-off, escalation). */
  ownerSms(): { to: string; body: string }[];
  /** SMS sent to the homeowner (Sarah's replies + escalation relays). */
  homeownerSms(): { to: string; body: string }[];
}
...
export async function startConversation(opts: ConversationOpts): Promise<Conversation> {
  requireApiKey();
  const contractor = opts.contractor ?? testContractor;
  ...
  return {
    ...
    homeownerPhone,
    async say(body) {
      const r = await handleInbound(deps, { toNumber, fromNumber: homeownerPhone, body, providerSid: `E2E-${++sid}` });
      return r.status === "ok" ? (r.reply ?? "") : "";
    },
    sayRaw(body, providerSid) {
      return handleInbound(deps, { toNumber, fromNumber: homeownerPhone, body, providerSid });
    },
    contractorReply(body, fromRecipientPhone) {
    ...
    async transcript() {
      ...
      return (ctx?.messages ?? [])
        ...
        .map((m) => `${m.direction === "inbound" ? "Homeowner" : "Sarah"}: ${m.body}`)
        .join("\n");
    },
    ownerSms: () => sms.sent.filter((s) => recipPhones.has(s.to)),
    homeownerSms: () => sms.sent.filter((s) => s.to === homeownerPhone),
  };
}
