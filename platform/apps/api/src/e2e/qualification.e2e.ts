...
describe.skipIf(!RUN_E2E)("E2E · qualification + homeowner hand-off (real Claude)", () => {
  it("in-area homeowner → qualified + owner gets a 'Qualified lead' ping", async () => {
  ...
  it("out-of-area + referral request → escalates (never invents a recommendation)", async () => {
    ...
    await c.say("ok no worries — can you recommend another roofer who covers my area?");
    expect(c.escalations().length).toBeGreaterThanOrEqual(1);
    expect(c.ownerSms().some((s) => s.body.includes("asks:"))).toBe(true);
  });

  it("tenant → asks for the owner's phone, then CODE pings the contractor with it (no booking)", async () => {
    const c = await startConversation({ contractor: easternContractor, now: SUNDAY });
    ...
    expect(
      c.ownerSms().some(
        (s) => s.body.includes("isn't the homeowner") && s.body.replace(/\D/g, "").includes("6175559090"),
      ),
    ).toBe(true);
    expect(c.appointments()).toHaveLength(0);
  });
