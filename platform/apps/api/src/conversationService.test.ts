...
/** Scripted-flow tests exercise the AGENT phase (post-intake), so flip the conversation to "agent".
 *  The scripted intake front is covered separately in intake/engine.test.ts. */
async function toAgentPhase(store: MemoryStore, leadId: string): Promise<void> {
  const ctx = await store.getContextByLeadId(leadId);
  await store.updateConversation(ctx!.conversation.id, { state: "agent" });
}

describe("conversation router + agent phase (SCOPE §5, §7.5)", () => {
  it("post-booking: a booked lead can text back to reschedule (conversation stays reachable)", async () => {
    ...
    // the booked lead texts back later — must still be found (state "agent", not "done")
