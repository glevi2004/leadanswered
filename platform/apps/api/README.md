...
- `POST /webhooks/twilio/sms` — inbound customer SMS. Runs the Sarah conversation engine: greet → qualify → propose slots → confirm → book, notifying the organization on booking.
...
Just an Anthropic key. Sarah's messages and organization notifications print to the console; data lives in memory.

1. `cp .env.example .env` and set `ANTHROPIC_API_KEY`.
2. From `platform/`: `pnpm install`
3. `pnpm dev` (boots with the seeded test organization "Apex Roofing").
4. Drive a conversation locally with curl:

```bash
# create a lead → Sarah texts the customer (logged to the console)
curl -s -X POST http://localhost:3000/lead -H 'content-type: application/json' \
  -d '{"name":"Jane Customer","phone":"+15551234567","project":"roof leak"}'

# simulate the customer texting back (To = organization's number, From = the lead)
...
3. `pnpm --filter @leadanswered/api seed` — insert the test organization + recipients.
...
- **AI orchestrates, TOOLS decide** (SCOPE §5.1): the model reasons and calls tools (`qualify_lead`, `get_availability`, `book_appointment`, `reschedule_appointment`, `cancel_appointment`, `escalate_to_organization`); every qualify/book/reschedule decision is deterministic code inside the tool, and the tool result is authoritative.
