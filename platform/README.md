...
> Status: **all three services are live in production** — `apps/api` (the "Sarah" tool-using agent, Vercel AI SDK, provider-agnostic) + `apps/worker` (BullMQ nudge) on Railway, `apps/web` (onboarding + admin, email+password invite-only auth) on Vercel, `packages/db` (Prisma) on Supabase. See `SCOPE.md` → "Architecture update (v2)" for the agent design and [`DEPLOY.md`](./DEPLOY.md) for the infra. Next up: the contractor dashboard (Phase 4).
