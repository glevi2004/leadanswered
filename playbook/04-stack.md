...
- Full control of the conversation agent and the **AI-orchestrates / tools-decide** qualification logic — the model picks tools, but every coverage/booking decision is deterministic code (no black-box automation builder to fight).
...
| ORM / DB | **Prisma** + **Postgres** (**Supabase**) | One Postgres for all data |
| SMS | **Twilio** (Node SDK) | Delivery + per-contractor dedicated numbers |
| AI | **Vercel AI SDK** (provider-agnostic) | The tool-using "Sarah" agent; defaults to Claude, OpenAI swap-ready |
| Observability | **Langfuse** (OpenTelemetry) | Traces every agent turn + tool call |
